...
// useful math macros 
#define BIT(n)  (1u<<(n)) // dispays number as bit in corresponding position
#define STEP(n) (100 * n) // input distance in mm, converts to number of steps, halfstepping 1 mm = 100 steps

//xy motor stuff
#define MOTOR_PIO pio0
#define MOTOR_AWO_PIN 5 //pico w pin 7, connected to both motors
...
// #define MOTOR_DREQ_DRIVER DREQ_PIO0_TX2 // pio0 sm2 tx fifo
...
// simple state machine for whole program
typedef enum
{
    e_ready,
    e_stitching,
...
// volatile extern bool *drReady;

extern Motor *xMotor;
extern Motor *yMotor;
// extern Motor *drMotor;
...
void abort_stitching(Stitching_State *state);
